// lib/ResumeAIClient.js
// ResumeAI DeepSeek Orchestrator – simple Node.js client wrapper
// Works with: http://localhost:5001/api/health and /api/deepseek/chat

import axios from 'axios';

const DEFAULT_BASE_URL =
  process.env.ResumeAI_BASE_URL || 'http://localhost:5001';
...
export async function ResumeAIHealth(config = {}) {
  const client = new ResumeAIClient(config);
  return client.health();
}

export async function ResumeAIChat(promptOrMessages, config = {}) {
  const client = new ResumeAIClient(config);
